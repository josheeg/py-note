...
1. **Folder+id dispatch** (`{spec_folder}` and `{story_id}` are set): the write-back always lands at the id-keyed story spec. The `C:\Users\joshe\Documents\game-note-py/_bmad-output/implementation-artifacts` fallback in step 2 below is never used in this mode, even for halts before planning starts.
...
2. **Otherwise:**
   ...
   - If `{spec_file}` is unknown or missing, create `C:\Users\joshe\Documents\game-note-py/_bmad-output/implementation-artifacts/bmad-build-auto-result-<slug-or-timestamp>.md` with:
     ```markdown
     ---
     status: <final status>
     ---
...
- Speak in `English`, tailor communication to `intermediate`, and write documents in `English`.
...
Read fully and follow: `C:\Users\joshe\Documents\game-note-py\_bmad\render\bmad-build-auto\game-note-py-e8af91dfec7c\20dfea06c8ae298bf0fd\step-01-clarify-and-route.md`.
